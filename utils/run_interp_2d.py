...
from visualize_utils import visual_for_loss_surface
...
def main():
    ...
    if args.assist_adapter:
        ...
        adapter_state_dict = torch.load(args.assist_adapter, map_location='cpu')
    ...
    if args.load_assist_adapter_to_init:
        print("Load train and student layer indexs from:", args.load_assist_adapter_to_init)
        metadata = srsly.read_json(
            args.load_assist_adapter_to_init.replace("adapter.pt", "metadata.json"))
        train_layer_indexs = [
            int(x) for x in metadata["train_layers"].split(",")
        ]
        layers = get_layers(init_model)
        print("Get train_layers:", train_layer_indexs)
        # assign adapter to teacher
        adapter_layers = torch.nn.ModuleList(
            [layers[index] for index in train_layer_indexs])

        print("Load adapter from:", args.load_assist_adapter_to_init)
        adapter_state_dict = torch.load(args.load_assist_adapter_to_init, map_location='cpu')
        adapter_layers.load_state_dict(adapter_state_dict)
        for index, layer in enumerate(train_layer_indexs):
            print("Assign", index, "to", layer)
            layers[layer] = adapter_layers[index]

        set_layers(init_model, layers)
    ...
    start, end = -4, 4
    points = 50
    progress_bar = tqdm(range(points**2))
    for x in np.linspace(start, end, points):
        for y in np.linspace(start, end, points):
    # for x in [0, 1]:
    #     for y in [0, 1]:
            # \theta_0 + \alpha * (\theta_1 - \theta_0) + \beta * (\theta_2 - \theta_0)
            # (0, 0) -> theta_0, (1, 0) -> theta_1/ft, (0, 1) -> theta_2/ot
            # progress_bar.set_description(
            # f"t_0 + ({x}) * (t_1 - t_0) + ({y}) * (t_2 - t_0)")
            for (p, p1, p0, p2) in zip(model.parameters(),
                                       finetuned_model.parameters(),
                                       init_model.parameters(),
                                       assist_model.parameters()):
                p.data = p0.data + x * (p1.data - p0.data) + y * (p2.data -
                                                                  p0.data)
            data["X"].append(x)
            data["Y"].append(y)
            ...
            progress_bar.set_description(f"X: {x} - Y: {y} - Z: {z}")
    df = pd.DataFrame(data)
    df.to_csv(f"./metadata/loss_landscape/{args.additional_note}.csv")
    with open(f"./metadata/loss_landscape/{args.additional_note}.json", "w") as f:
        f.write(json.dumps(data))
        f.write("\n")
    # visual_for_loss_surface(df, args.additional_note, strs=args.point_strs)
    ...
    # visual_for_loss_surface(df, args.additional_note, strs=args.point_strs)
    # df.to_csv(f"./metadata/loss_landscape/{args.additional_note}.csv")
    with open(f"./metadata/loss_landscape/{args.additional_note}_4points.json", "w") as f:
        f.write(json.dumps(data))
        f.write("\n")
